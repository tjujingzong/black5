...
- 实时语音编码为短 Opus 音频片段，由 Cloudflare Durable Object 通过房间 WebSocket 广播；无需 P2P、STUN 或 TURN，跨手机运营商网络也能使用。
...
- 任一阵营全部出完时提前结束并补齐剩余名次；双方阵营名次分相抵时全员记 `0` 分，否则按阵营人数加权进行严格零和结算，独庄分数翻倍。
